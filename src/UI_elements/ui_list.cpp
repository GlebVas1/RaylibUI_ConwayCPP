...
void UIList::Init() {
    list_render_texture_ = std::make_shared<RenderTexture2D>(LoadRenderTexture(width_ - slider_box_x_space_, elements_.size() * y_item_size_));
}
...
void UIList::Draw() {
    ...
    for (size_t i = 0; i < elements_.size();  ++i) {
        ...
        DrawRectangle(0, i * y_item_size_, width_ - slider_box_x_space_, y_item_size_ - y_offset, item_color);
        DrawRectangleLines(0, i * y_item_size_, width_ - slider_box_x_space_, y_item_size_ - y_offset, ui.ui_line_color);
    ...
    UITools::DrawRectangle(
        ...
        GetAbsoluteYPosition() + slider_line_y_offset_ + static_cast<int>(slider_position_ * (height_ - 2 * slider_line_y_offset_)),
        slider_width_,
        slider_height_,
        2,
        0.3f,
...
void UIList::Update() {
    ...
    bool mouse_on_slider = CheckCollisionPointRec(GetMousePosition(), Rectangle{
        ...
        static_cast<float>(GetAbsoluteYPosition() + slider_line_y_offset_ + static_cast<int>(slider_position_ * (height_ - 2 * slider_line_y_offset_))),
        static_cast<float>(slider_width_),
        static_cast<float>(slider_height_)
    });
